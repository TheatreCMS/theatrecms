...
if (isset($app)) {
    $app->group('/admin/posts', function ($group) {
        ...
        $group->delete('/{id}/hero-image', [PostController::class, 'removeHeroImage']);
        $group->delete('/{id}',   [PostController::class, 'destroy']);
        $group->get('',           [PostController::class, 'index']);
    })->add(new RequireTwigMiddleware($container))
      ->add(new RequireCapabilityMiddleware($container->get(AuthorizationService::class), Capability::EDIT_POSTS))
