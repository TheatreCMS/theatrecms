...
| `theatrecms/the_featured_image_url` | Fired by the `the_featured_image_url()` Twig function (see `src/Twig/FeaturedImageExtension.php`), TheatreCMS's corollary to WordPress' `the_post_thumbnail_url()`. Resolves the stored featured image URL for whatever content entity is passed to it (any model exposing `getFeaturedImageUrl()`, e.g. `Production`/`Post`/`Season`), returning `''` when the entity has none, then lets themes filter the result. | `string` (the resolved URL, or `''`) | The entity passed to `the_featured_image_url()` |
